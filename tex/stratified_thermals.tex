...
We estimate that solar downflows should be in the falling regime and should remain large enough to shield their entropic signature from diffusive effects until they reach the base of the solar convective zone.
\end{abstract}
...
However, the vortex rings described by \citet{brandenburg2016} do not include a fundamental aspect of entropy rain: it has lower entropy than the background atmosphere.
Entropy rain is dense, and buoyancy forces will modify its dynamics.

It is important to understand how the filling fraction of these basic convective elements is affected by their negative buoyancy. 
...
Such an expansion may prevent entropy rain from reaching the bottom of the solar convection zone.
...
\begin{figure}[t!]
    ...
    \caption{
	Entropy perturbations, scaled by density and thermal volume and normalized by the thermal's buoyancy perturbation, are shown.
	...
	Shown are (a) a thermal in a weakly stratified domain with $n_\rho = 1/2$ density scale heights, and (b) a thermal in a strongly stratified domain with $n_\rho = 3$.
	While both start with precisely the same initial condition, the thermal in low stratification expands with depth, whereas the thermal in strong stratification compresses marginally with depth.
...
\begin{figure}[t!]
	...
	With increasing stratification, we see marginally more detrainment (a).
    \label{fig:constants} }
\end{figure}
