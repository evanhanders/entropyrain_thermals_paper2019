...
\end{equation}
where $\beta$ is due to the ``virtual mass effect'' in which the thermal drags environment air along with it \citep{tarshish&all2018}.
...
From this virtual origin, the vortex ring's impulse grows $I_0$ at time $t = 0$ when the true thermal is released from rest.
...
We thus express the negative buoyancy as 
\begin{equation}
...
\label{eqn:r_theory}
\end{equation}
Recall there are no net baroclinic torques, so stays $\Gamma_{\text{th}}$ nearly constant (Fig.~\ref{fig:constants}b).
In the Boussinesq limit where $\rho \rightarrow \text{constant}$, we retrieve the $r \propto \sqrt{t}$ scaling found in \LJ.
In the limit of strong stratification, $r \propto \rho^{-1/2}$. 
In the absence of buoyancy ($B_{\text{th}} = 0$), this corresponds to purely horizontal compression, with $r^2 \propto \rho^{-1}$ \citep{brandenburg2016}.
...
\end{equation}
Defining the thermal velocity $w_{\text{th}} \equiv dz_{\text{th}}/dt$, and assuming $I_0$ is small so can be neglected, we have \dl{fix this equation}
\begin{equation}
...
We assume the thermal's volume is spheroidal, and that the virtual mass effect and detrainment can be parameterized as constants.
Under these assumptions, we arrive at Eqn.~\ref{eqn:dz_theory}.
