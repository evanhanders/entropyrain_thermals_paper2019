...
\textbf{\citet{brandenburg&hazlehurst2001} found that hot, buoyant thermals in stratified domains behave qualitatively similar to Boussinesq thermals.
...
The 2D simulation domain is \textbf{periodic in the z-direction with $z \in [-L_z/4, L_z]$ and the radial direction spans $r \in [0, L_r]$}.
...
The 3D simulation domain is \textbf{periodic in the horizontal directions ($x, y \in [-L_r, L_r]$) and vertically spans $z \in [0, L_z]$}.
...
\textbf{
In our 2D simulations, we represent the radial direction with Chebyshev polynomials so that we can include geometric factors in ours equations and capture the singularity at $r = 0$.
...
In the first and second panels of Fig. \ref{fig:thermal_sun_comparison}, we show the evolution of these thermals' radii and velocities inside of the Sun, and compare their evolution to pure horizontal compression.
...
Following \citet{brandenburg2016}, we estimate the enthalpy flux of the thermal as $F_{\text{enth}} = \rho_0(z_th) T_0(z_th) w_{\text{th}} S_{1,\text{th}}$.
We estimate the thermal's entropic signature as $S_{1, th} = B_{\text{th}}/(\rho_0(z_{\text{th}}) V_{\text{th}} g/c_P)$, where the thermal volume is $V_{\text{th}} = m r_{\text{th}}^3$.
...
To estimate the total luminosity carried by one thermal, we calculate $L_{\text{enth, th}} = \pi r_{\text{th}}^2 F_{\text{enth}}$, and we plot this value as a function of depth, normalized by the solar luminosity, in panel 4 of Fig.~\ref{fig:thermal_sun_comparison}.
}


\textbf{
From our estimate of the luminosity carried by one thermal, we estimate the filling factor of downward propagating thermals required to carry the solar luminosity through their enthalpy flux.
We calculate $f_{\text{th}} = (L_{\odot}/L_{\text{enth, th}}) (\pi r_{\text{th}}^2)/(4\pi R^2)$, where $R$ is the thermal's radial distance form the center of the Sun.
We plot the $f_{\text{th}}$ in panel 5 of Fig.~\ref{fig:thermal_sun_comparison}.
...
A few percentage of the solar radius beneath the photosphere, $f_{th}$ drops to a very modest $10^{-4}$, and by the base of the solar convection zone approaches $10^{-7}$.
These estimates suggest that even if a large fraction of the thermals launched from the solar surface break apart due to turbulence, this mechanism could still efficiently carry the solar luminosity deep in the convection zone.
}

\textbf{
While these estimates suggest that entropy rain could effectively transport the solar luminosity, we will briefly note that our handling of the enthalpy flux here ignores the contributions of the kinetic energy flux, potential energy flux, and viscous flux.
...
A more thorough examination of these fluxes are outside of the scope of this work, but due to the promising nature of our results found here for the entropy rain hypothesis, such work seems promising.
}
...
\textbf{We'd like to further thank Nadir Jeevanjee, Axel Brandenburg, Jeremy Goodman, and the anonymous referee whose careful comments greatly improved the scientific quality and clarity of this manuscript.}
...
\textbf{3}	 	& 		\textbf{384}			& \textbf{2048}			& \textbf{25}    	&	\textbf{0.15}	\\
\enddata																																															
\tablecomments{
