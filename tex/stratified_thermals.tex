...
We verify the theory with 2D \textbf{cylindrical} and 3D \textbf{cartesian} simulations of laminar, axisymmetric thermals in highly stratified atmospheres.
...
\textbf{In this work we study vortex rings generated by discrete cold thermals, but ``plumes'' driven by time-stationary cooling produce similar vortex ring structures \citep[as in e.g.,][]{rast1998}.
...
\begin{figure}[t!]
    ...
    \caption{
	\textbf{Shown is the evolution of entropy perturbations, normalized by the thermal's buoyancy perturbation, from 3D simulations conducted in this work}.
...
\textbf{This is the appropriate regime of deep solar convection.}
...
\label{eqn:r_theory}
\end{equation}
\textbf{Here, $\Gamma_{\text{th}}$ is the characteristic circulation of the thermal. As} there are no net baroclinic torques, $\Gamma_{\text{th}}$ \textbf{remains nearly constant in our simulations} (Fig.~\ref{fig:constants}, middle panel).
...
\textbf{Here, $m$ accounts for volumetric constants (e.g., $4\pi/3$), the aspect ratio of the thermal, and the cubed ratio between the full radius of the spheroidal thermal and $r$.}
...
Following \LJ\,, we non-dimensionalize the equations \textbf{on} the initial diameter of the thermal and its freefall velocity.
...
We choose an atmospheric model in which the dynamic viscosity, $\mu = \rho_0 \nu$, and the thermal conductivity, $\kappa = \rho_0 \chi$, are both uniform \textbf{in space} and constant in time.
...
All simulations conducted in this work use \textbf{an initial} value of Re = 600 and $\mbox{\text{Pr} = 1}$.
...
\end{equation}
\textbf{In all cases, we do not add any symmetry breaking perturbations (e.g., noise).}
...
Outside these times, we find differences in the \textbf{measured} radius of less than 1\%.
...
\startdata																																															
\multicolumn{8}{l}{\textbf{2D Anelastic Simulations (Cylindrical)}}\\
...
\multicolumn{8}{l}{\textbf{3D Fully Compressible Simulations (Cartesian)}}\\    
...
We also find that the non-dimensional circulation is \textbf{roughly} $-2$ for each of our cases, and decreases in magnitude with increasing stratification.
...
Using a solar surface gravity value of $g = 2.74 \times 10^4$ cm/s$^2$ \textbf{and assuming thermal formation occurs at the solar photosphere}, the buoyancy of a spherical thermal is 
$$
...
In the first and second panels of Fig. \ref{fig:thermal_sun_comparison}, we show the evolution of these thermals' radii and velocities inside of the Sun, and compare their \textbf{radial} evolution to pure horizontal compression \textbf{and their velocity evolution to the speed of sound}.
...
We find that our estimated solar thermal is \textbf{likely} in the falling regime.
...
\textbf{The 3D, $N_\rho = 3$ simulation displays spectral instabilities at very late times as the simulation is under-resolved at these times.
