...
nondim  = rho_top * L**2 * u_th**2 * np.abs(S1_over_cp)
...
for Buoy in (B_nondim/2, 2*B_nondim):
    ...
    T0 = 1 + grad_T_ad*(z0-20)

    Temps = theory_T(times, B0, Gamma, m, chi, k, t_off, T0, grad_T_ad=grad_T_ad, m_ad=m_ad)
    Rhos  = Temps**(m_ad)
    ws    = theory_dT_dt(times, B0, Gamma, m, chi, k, t_off, T0, grad_T_ad=grad_T_ad, m_ad=m_ad)/grad_T_ad
...
#for i, bounds in enumerate(((3e-2, 1e-1, 0.66, 0.705), (4.8e-2, 1e-1, 0.66, 0.705), (1e8, 4e8, 0.66, 0.705))):
#    bot, top, left, right = bounds
#    axs[i].plot((left, right), (bot, bot), c='k', lw=1)#0.75)
#    axs[i].plot((right, right), (bot, top), c='k', lw=1)#0.75)
#
#axs[0].text(0.685, 4e-2, 'a',   fontsize=15)#, weight='bold')
#axs[1].text(0.684, 5.5e-2, 'b', fontsize=15)#, weight='bold')
#axs[2].text(0.685, 1.35e8, 'c', fontsize=15)#, weight='bold')
